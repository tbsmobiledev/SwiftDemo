...
@IBDesignable
class UnderLineTextField: SITextField {
...
extension UnderLineTextField {
    ...
    private func setPlaceholderPlace(isUp: Bool, isAnimated: Bool) {
        if isUp {
            let scaleTransform = CGAffineTransform(scaleX: 0.8, y: 0.8)  //Addded 0.8 -> 0.7
            let dummy = UIView()
    ...
    private func createLinePath() -> UIBezierPath {
        ...
        path.move(to: CGPoint(x: 0, y: padding + 3))  //Added +3
        path.addLine(to: CGPoint(x: bounds.maxX, y: padding + 3))  //Added +3
        return path
    }
    
    private func textHeightForFont(font: UIFont?) -> CGFloat {
        let font = font ?? self.font ?? .systemFont(ofSize: 16 * DeviceScale.SCALE_Y)
